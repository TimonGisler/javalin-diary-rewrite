...
fun getJavalinApp(): Javalin {
    ...
    return Javalin.create { config ->
        config.plugins.enableCors { it.add { it.anyHost() } } //TODO TGIS; REMOVE!!!!! ONLY FOR DEV --> OR BETTER YET add env variable if env = dev enable this cors policy : )
        config.staticFiles.add("/public")
        config.accessManager(AccessManager())
    }
        .routes {
            path("/api") { // TODO TGIS, change prefix to /api
